...
/**
 * ═══════════════════════════════════════════════════════════════════════════
 * THE SIX DAMAGE TYPES NOW LIVE IN `src/shared/damagetype.ts`.
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * They moved because the log has to SAY them: `DamageEvent` carries the type
 * from PROTOCOL_VERSION 20, and `src/shared/` is the only module a wire type, a
 * server engine and a browser renderer may all import. The block that defined
 * them here already gave "the client's log renderer" as the reason its values
 * are lowercase — a reason that was not yet true of any code.
 *
 * RE-EXPORTED RATHER THAN RELOCATED-AND-REWRITTEN, because a hundred files
 * import `DamageType` from this module and the damage MATHS is still here. One
